...
export default function EmergencyPage({ data, page }) {
    ...
    return (
        <div className='main-section w-full flex flex-col gap-4 md:gap-6 lg:gap-8 text-[--color-texts]'>
            ...
            <div className='boxes grid grid-rows-2 grid-cols-2 w-[90%] mx-auto gap-4'>
                {emergencyContacts.map((item, index) => (
                    <div
                        key={index}
                        className='boxaki flex flex-col justify-center items-start bg-[--color-secondary] p-2 border rounded-xl'
                    >
                        <h2 className='font-bold mb-0'>{item?.title}</h2>
